...
class AttackSimulationApp:
    ...
    def setup_styles(self):
        ...
        self.mono_font = ("Noto Sans CJK SC", 12)
    ...
    def setup_layout(self):
        ...
        self.ai_input = tk.Text(self.ai_panel, height=1, font=self.mono_font, bg=self.fg_color, fg=self.text_color)
        self.ai_input.pack(fill="x", padx=5, pady=5)
        self.ai_input.bind("<Return>", self.process_security_query_text)
    ...
    def process_security_query_text(self, event):
        query = self.ai_input.get("1.0", "end-1c").strip()
        if query:
            try:
